...
{
    mySerial.printf("Logic: Checking initialization...\n");
    bool initialized = true;

    if (!myLed.isInitialized()) { mySerial.printf("Logic: LED not initialized!\n"); initialized = false; }
    if (!myToggleButton.isInitialized()) { mySerial.printf("Logic: ToggleButton not initialized!\n"); initialized = false; }
    if (!myTempButton.isInitialized()) { mySerial.printf("Logic: TempButton not initialized!\n"); initialized = false; }
    if (!myDebounceTimer.isInitialized()) { mySerial.printf("Logic: DebounceTimer not initialized!\n"); initialized = false; }
    if (!myToggleTimer.isInitialized()) { mySerial.printf("Logic: ToggleTimer not initialized!\n"); initialized = false; }
    if (!myTempTimer.isInitialized()) { mySerial.printf("Logic: TempTimer not initialized!\n"); initialized = false; }
    if (!mySerial.isInitialized()) { mySerial.printf("Logic: Serial not initialized!\n"); initialized = false; }
    if (!myWatchdog.isInitialized()) { mySerial.printf("Logic: Watchdog not initialized!\n"); initialized = false; }
    if (!myEeprom.isInitialized()) { mySerial.printf("Logic: EEPROM not initialized!\n"); initialized = false; }
    if (!myTempSensor.isInitialized()) { mySerial.printf("Logic: TempSensor not initialized!\n"); initialized = false; }

    if (initialized) { mySerial.printf("Logic: All components initialized successfully!\n"); }
    else { mySerial.printf("Logic: Initialization FAILED for one or more components.\n"); }
...
void Logic::handleButtonEvent() noexcept
{
    ...
    const bool toggleButtonPressed = myToggleButton.read();
    const bool tempButtonPressed = myTempButton.read();

    if (toggleButtonPressed || tempButtonPressed)
    {
        ...
        if (toggleButtonPressed) { handleToggleButtonPressed(); }
        if (tempButtonPressed) { handleTempButtonPressed(); }
    }
}
...
void Logic::handleToggleTimerTimeout() noexcept 
{
    ...
    if (myToggleTimer.hasTimedOut() && myToggleTimer.isEnabled()) { myLed.toggle(); }
}
