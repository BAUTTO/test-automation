...
TEST(Logic, ToggleHandling)
{
    ...
    {
        bool ledState = mock.led.read();
        mock.tempTimer.setTimedOut(true);
        logic.handleTempTimerTimeout();
        EXPECT_EQ(ledState, mock.led.read());
    }
...
TEST(Logic, TempHandling)
{
    ...
    mock.tempSensor.setTemperature(9000);
    const auto initialPrintCount = mock.logicImpl->tempPrintoutCount();
    ...
    // Expect the temperature to not be printed, since the wrong button was pressed.
    {
        ...
        EXPECT_EQ(initialPrintCount, mock.logicImpl->tempPrintoutCount());
    }
    ...
    // Expect the temperature to be printed once.
    {
        ...
        EXPECT_EQ(initialPrintCount + 1, mock.logicImpl->tempPrintoutCount());
    }
    ...
    // Expect the temperature to be printed once more.
    {
        ...
        EXPECT_EQ(initialPrintCount + 2, mock.logicImpl->tempPrintoutCount());
    }
}
